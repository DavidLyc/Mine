...
public class RectifyDoingPresenter implements RectifyDoingContract.Presenter {

    private RectifyDoingContract.View mView;
    ...
    private final static String Rectify_ConfirmState = "1";
    private Boolean mIsFirstLoad = true;

    RectifyDoingPresenter(@NonNull RectifyDoingContract.View view, @NonNull Context context) {
        mView = view;
        mContext = context;
    ...
    @Override
    public void loadDoingAlarmInfo() {
        mDisposables.add(
    ...
    public void saveRectifyInfo(List<RectifyListItem> items) {
        ...
        String retractionForAlarmCompletedTime = TimeUtils.getTodaynyrsfm();
        for (RectifyListItem item : items) {
            JsonArray temp = new JsonArray();
            item.setRectifactionForAlarmCompletedTime(retractionForAlarmCompletedTime);
    ...
    public Boolean loadMoreCacheData() {
        if (mJsonArray != null && mNowItemNum < mTotalItemNum) {
            ...
            mView.showDoingAlarms(itemList);
            return true;
        }
        return false;
    }
